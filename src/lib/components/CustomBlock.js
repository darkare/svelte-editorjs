import CustomBlockComponent from './CustomBlock.svelte';

class CustomBlock {
  static get toolbox() {
    return {
      title: 'Custom Block',
      icon: '<img id="dimg_41" src="data:image/png;base64," style="border-radius:4px 4px 4px 4px" class="YQ4gaf zr758c wA1Bge" height="32" width="32" alt="" data-csiid="13" data-atf="1">',
    };
  }

  constructor({ data, api }) {
    this.data = data;
    this.api = api;
  }

  render() {
    const container = document.createElement('div');
    const customBlock = new CustomBlockComponent({
      target: container,
      props: {
        data: this.data,
      },
    });
    return container;
  }

  save(blockContent) {
    const input = blockContent.querySelector('input');
    return {
      content: input.value,
    };
  }
}

export default CustomBlock;
